    ...
    this.chatService.sendMessage(messageText)
      ...
      .subscribe({
        ...
        error: (error: any) => {
          ...
          if (status === 404) {
            errorText = 'El servidor del chatbot no está disponible. Por favor, verifica que el backend esté corriendo en http://localhost:3010';
          } else if (status === 0 || !status) {
            errorText = 'No se pudo conectar al servidor. Verifica que el backend esté corriendo en http://localhost:3010';
          }
